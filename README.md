...
- `search_web(query, limit=10, lang="en", region=None, safe_search=True)`: searches the web via googlesearch-python (Google scrape) and returns up to `limit` results with title, URL, and description. Results are capped to 25 to limit requests.
